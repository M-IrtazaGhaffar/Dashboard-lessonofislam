...
function Edit() {
  ...
  const fetchBlog = async () => {
    try {
      setLoading(1)
      const res = await axios.post(`https://api-lessonofislam.ittidevelops.com/writer/fetchBlog`, {
        token: token,
        id: data
      })
                ...
                data ?
                  <form onSubmit={UploadData}>
                    ...
                    <div>
                      ...
                      <textarea rows={5} name='title' value={Title} onChange={(e) => setTitle(e.target.value)} maxLength={250} required></textarea>
                      <p className='length'>{`${Title.length}/250`}</p>
                    </div>
                    <span>
                      <label htmlFor="description">Description</label>
                      <textarea rows={10} name='description' value={Descr} onChange={(e) => setDescr(e.target.value)} maxLength={250} required></textarea>
